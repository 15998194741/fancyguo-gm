...
export class GmDictController {
  ...
  async findById(ctx) {
  	...
  	const result = await gmDictService.findById(ctx.params.id);
  	ctx.body = statusCode.SUCCESS_200('查找成功', result);
  }
  @post('/')
  async create(ctx) {
  	...
  	const result = await gmDictService.create(ctx.request.body);
  ...
  async update(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmDictService.updateById(ctx.request.body);
  ...
  async delete(ctx) {
  	...
  	if(!ctx.user) throw statusCode.ERROR_501('用户未登录');
  	const result = await gmDictService.logicDeleteByIdToUserId(ctx.request.body.id, ctx.user);
