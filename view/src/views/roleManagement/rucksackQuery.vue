<template>
  <div class="ruck-container">
    ...
    <ul style="margin: 5px 5px -5px 0;">
      ...
      <li> <el-button  v-if='grade' slot="append" :disabled='selectmanyone' icon="el-icon-delete-solid" size='small' class="button-with-header"  @click='dialogFormchange = true'>道具回收</el-button></li>
    </ul>
  </div>
...
export default {
  ...
  computed: {
    ...
    selectmanyone() {
      return this.allselectchange.length > 0;
    }
  },
  methods: {
    ...
    async recycleAmountPush() {
      if (this.allselectchange.length < 1) {return;}
      let mergetrue = await this.$confirm('是否确认回收道具?', '提示', {
